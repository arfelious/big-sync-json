...
class FastBuffer extends Uint8Array {
  ...
  customSlice= function(buffer,start, end) {
...
let matchWhileValid = (bufferToParse, index = 0, increaser, decreaser = null) => {
    ...
    //number
    if ((firstChar > 47 && firstChar < 58)||firstChar==45) {
...
let getNextKey = (bufferToParse, index = 0) => {
    ...
    return [validNext&&customSlice(validNext[0], 1,validNext[0].length-1),countSpaces]
}
let getNextValue = (bufferToParse, index = 0) => {
    let i = index;
    while(bufferToParse[i] == 32 || (bufferToParse[i] > 8 && bufferToParse[i] < 14))i++
    let countStartingSpace = i
...
let getNextNumber = (bufferToParse, index = 0, byItself = false) => {
    ...
    for (let i = index; i < bufferLength; i++) {
        let curr = bufferToParse[i]
        let isInvalid = ((isStartingWithZero && i > index &&bufferToParse[index+1]!=46&&bufferToParse[index+1]!=69&&bufferToParse[index+1]!=101)||curr==47||curr < 45 || curr > 57) || ((curr == 46 ? fractionIsUsed?true:!(isFraction = !isFraction,fractionIsUsed=true) : false))||curr==45&&i>index
...
let getNextObject = (bufferToParse, index = 0) => {
    ...
    let i;
    for (i = index+1; i < bufferLength-2; i++) {
        ...
        let add = nextKeyFirst.length + nextKey[1] + 2 + nextValue[0].length + nextValue[1]
        let until = untilNextKey(bufferToParse, i + add)
        if (!until) break
        i += add + until
    }
    if(bufferLength-i==1)_throw("Trailing commmas are not allowed.", "getNextObject")
    return o
}
...
let valueOf = (bufferToParse) => {
    ...
    if (firstElement == 34) {
        ...
        return result ? customSlice(result, 1, bufferLength - 1).toString().replace(/(?<=([^\\]|^)(\\\\)*)\\([btrnf])/g,q=>{
            return escapeReady[q[1]]
        }).replace(/\\\\/g,"\\")
        .replace(/\\"/g,'"'): _throw("Invalid String", "valueOfValidString")
    }
    //isNumber
    else if (((firstElement > 47 && firstElement < 58)||firstElement==45)) {
